module Adjective
  class Inventory
    ...
    def find_by_instance_id(id)
      @grouped_items[id]
    end

    def sort
      return @items.sort_by { |item| item.instance_id }
    end
    ...
    def sort!
      @items = @items.sort_by { |item| item.instance_id }
    end
    ...
    def sort_grouped_items_by!(attribute)
      raise RuntimeError, "'#{attribute}' is not present on an item set: #{items}" if items.any? {|item| !item.respond_to?(attribute)} 
      @grouped_items.each do |id, items|
        @grouped_items[id] = items.sort_by {|item| item.send(attribute) }
      end
    ...
    def group_items(items)
      ...
      if (items.any? {|item| !item.respond_to?(:instance_id)} || items.any? {|item| item.nil? })
        raise RuntimeError, "Given item is invalid. Ensure that the item has an id assigned and is not nil: #{items}" 
      end

      grouped_items = {}
      items.each do |item|
        if !grouped_items.key?(item.instance_id)
          grouped_items[item.instance_id] = [item]
        elsif grouped_items.key?(item.instance_id)
          grouped_items[item.instance_id] << item
        end
      end
